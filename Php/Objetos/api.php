...
class ProxmoxVE_API
{
    ...
    public function login() {
        ...
        // Verificar si hay error en la conexión SSL
        if ($login_response['ssl_verify_result'] == 1) {
            throw new PVE_Exception("Incorrect SSL on {$this->hostname}", 4);
        }

        $response_data = json_decode($response, true);
        var_dump($response_data); // Verifica la respuesta completa de la API para el ticket

        // Asegurarse de que el ticket esté presente
        if (!isset($response_data['data']['ticket'])) {
            throw new PVE_Exception("No se pudo obtener el ticket de autenticación.");
        }

        $this->login_data = $response_data['data'];
    ...
    public function noVNC($node, $vmid) {
        ...
        // Verifica que la respuesta esté correctamente formada antes de usarla
        if (isset($conf['port'])) {
            ...
            // Verifica si la respuesta websock contiene datos
            if (isset($websock['port'])) {
                ...
                throw new PVE_Exception("Error al obtener la respuesta del WebSocket.");
            }
        } else {
            throw new PVE_Exception("Error al obtener el puerto de VNC proxy.");
        }
    }
}
?>
