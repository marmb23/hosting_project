<?php
    ...
    // Itera sobre cada màquina virtual o contenidor per encendre'ls
    foreach ($vms as $vm) {
        $node = $vm['node']; 
        $vmid = $vm['vmid']; 

        // Encén el contenidor utilitzant l'API de Proxmox
        $proxmox->startLXC($node, $vmid);
    }
    
    // Redirigeix a la pàgina anterior després de completar l'encesa
