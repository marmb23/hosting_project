<?php
    ...
    // Itera sobre cada màquina virtual o contenidor per eliminar-los
    foreach ($vms as $vm) {
        $node = $vm['node']; 
        $vmid = $vm['vmid']; 

        // Elimina el contenidor de la base de dades
        $database->deleteLXC($vmid);

        // Elimina el contenidor utilitzant l'API de Proxmox
